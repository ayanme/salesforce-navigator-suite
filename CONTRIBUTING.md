...
If the domain covers a product currently handled through cross-domain routing (Agentforce, Data Cloud, Marketing Cloud Personalization, CRM Analytics, MuleSoft), open an issue to discuss before starting.
...
- If the new Navigator covers a product currently handled through cross-domain routing, update the cross-domain routing table in `PROJECT_STANDARD.md §7` and remove the relevant scope entries from the affected Domain Navigators.
...
Domain Navigators own a single documentation type across all Salesforce products. When a Salesforce product spans multiple documentation domains, its documentation is distributed across the relevant Domain Navigators by documentation type.
...
Product Specialist Navigators may be proposed for any product with sufficient documentation complexity to justify a unified entry point.
...
When in doubt, prefer cross-domain routing through existing Domain Navigators over creating a new Product Specialist Navigator prematurely.

---

## Adding a Product Specialist Navigator for a Cross-Domain Product

When a new Product Specialist Navigator is created for a product currently handled through cross-domain routing, follow this process to transfer ownership cleanly and avoid gaps or conflicts.

### Step 1 — Remove the product from the Cross-Domain Routing section of PROJECT_STANDARD.md

Remove the product's entry from the Cross-Domain Routing table in §7 of PROJECT_STANDARD.md. The new Navigator definition becomes the canonical ownership record.
...
### Step 3 — Update affected Domain Navigator descriptions

Remove any cross-domain routing scope entries referencing this product from the affected Domain Navigator files.
